...
func (_q *RequestQuery) QueryGate() *GateQuery {
	query := (&GateClient{config: _q.config}).Query()
...
func (_q *RequestQuery) QueryResponses() *ResponseQuery {
...
// QueryDiff chains the current query on the "diff" edge.
func (_q *RequestQuery) QueryDiff() *DiffQuery {
...
func (_q *RequestQuery) Clone() *RequestQuery {
	...
	return &RequestQuery{
		...
		withGate:      _q.withGate.Clone(),
		withResponses: _q.withResponses.Clone(),
		withDiff:      _q.withDiff.Clone(),
...
func (_q *RequestQuery) WithGate(opts ...func(*GateQuery)) *RequestQuery {
	query := (&GateClient{config: _q.config}).Query()
...
func (_q *RequestQuery) WithResponses(opts ...func(*ResponseQuery)) *RequestQuery {
	query := (&ResponseClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
// WithDiff tells the query-builder to eager-load the nodes that are connected to
// the "diff" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *RequestQuery) WithDiff(opts ...func(*DiffQuery)) *RequestQuery {
	query := (&DiffClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *RequestQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Request, error) {
	...
	if query := _q.withGate; query != nil {
		if err := _q.loadGate(ctx, query, nodes, nil,
			func(n *Request, e *Gate) { n.Edges.Gate = e }); err != nil {
			return nil, err
		}
	}
	...
	if query := _q.withDiff; query != nil {
		if err := _q.loadDiff(ctx, query, nodes, nil,
			func(n *Request, e *Diff) { n.Edges.Diff = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *RequestQuery) loadGate(ctx context.Context, query *GateQuery, nodes []*Request, init func(*Request), assign func(*Request, *Gate)) error {
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "gate_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *RequestQuery) loadResponses(ctx context.Context, query *ResponseQuery, nodes []*Request, init func(*Request), assign func(*Request, *Response)) error {
...
func (_q *RequestQuery) loadDiff(ctx context.Context, query *DiffQuery, nodes []*Request, init func(*Request), assign func(*Request, *Diff)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Request)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
...
func (_q *RequestQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withGate != nil {
			_spec.Node.AddColumnOnce(request.FieldGateID)
		}
	}
