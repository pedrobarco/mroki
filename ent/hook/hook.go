...
// The DiffFunc type is an adapter to allow the use of ordinary
// function as Diff mutator.
type DiffFunc func(context.Context, *ent.DiffMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f DiffFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.DiffMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.DiffMutation", m)
}
...
func (f GateFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.GateMutation); ok {
		return f(ctx, mv)
	}
