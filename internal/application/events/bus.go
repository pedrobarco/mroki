...
// raise domain events implementing internal/domain/event.Event; the use case
...
type Dispatcher interface {
	Dispatch(ctx context.Context, events ...event.Event)
}
...
func (b *Bus) Dispatch(ctx context.Context, events ...event.Event) {
...
func (b *Bus) deliver(ctx context.Context, h Handler, evt event.Event) {
	defer func() {
		if r := recover(); r != nil {
