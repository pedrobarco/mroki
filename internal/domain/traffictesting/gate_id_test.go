...
func TestParseGateID_valid_uuid(t *testing.T) {
	...
	id, err := traffictesting.ParseGateID(input)
...
func TestParseGateID_invalid_uuid(t *testing.T) {
	...
	assert.ErrorIs(t, err, traffictesting.ErrInvalidGateID)
	assert.True(t, id.IsZero())
}

func TestNewGateID_creates_unique_ids(t *testing.T) {
	id1 := traffictesting.NewGateID()
	id2 := traffictesting.NewGateID()
...
func TestGateIDFromUUID_roundtrip(t *testing.T) {
	...
	gateID := traffictesting.GateIDFromUUID(original)
...
func TestGateID_IsZero(t *testing.T) {
	var zero traffictesting.GateID
	assert.True(t, zero.IsZero())

	nonZero := traffictesting.NewGateID()
